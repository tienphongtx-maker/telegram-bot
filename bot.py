...
async def taixiu(update, ctx):
    ...
    try:
        ...
        return await update.message.reply_text("Dùng: /tx 1000 tai")
    ...
    if choice not in ["tai", "xiu"]:
        return await update.message.reply_text("Chọn tai hoặc xiu")

    # chống spam
    if uid in last_game and time.time() - last_game[uid] < 10:
        return await update.message.reply_text("⏳ Chờ 10 giây")

    last_game[uid] = time.time()

    roll = random.randint(1, 6)
    result = "tai" if roll >= 4 else "xiu"

    if choice == result:
        add_money(uid, amount, "win_tx")
        await update.message.reply_text(f"🎉 Ra {roll} → {result}\n+{amount}")
    else:
        sub_money(uid, amount)
        await update.message.reply_text(f"💀 Ra {roll} → {result}\n-{amount}")
===== RUN =====
...
app.add_handler(CommandHandler("tx", taixiu))
